...
const comments = [
  { id: 1, author: "Dr. Chen", initials: "DC", time: "1h ago", text: "Fascinating analysis. The correlation between transformer self-attention and human selective attention mechanisms is highly underexplored. However, biological networks exhibit much higher sparsity. How do you account for that in your model?", upvotes: 24, downvotes: 3 },
  { id: 2, author: "Alex Morgan", initials: "AM", time: "45m ago", text: "Great read! I'd add that the energetic constraints on biological brains might be the exact reason they evolved such efficient sparse attention mechanisms compared to our current dense AI models.", upvotes: 18, downvotes: 1 },
  { id: 3, author: "Sophia R.", initials: "SR", time: "15m ago", text: "This bridges the gap perfectly. Would love to see a follow-up exploring how this framework applies to recurrent neural networks and memory retention.", upvotes: 8, downvotes: 0 },
];
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          ...
          <div className="flex flex-col items-end gap-4">
            <button className="flex items-center gap-2 bg-[#0F0F1A] text-white px-5 py-3 rounded-full font-semibold hover:bg-[#4040FF] transition-colors shadow-lg">
            ...
            <div className="flex gap-2 bg-white p-1 rounded-full shadow-sm border border-gray-100 text-sm font-medium">
              {["All", "Academia", "Self-Dev", "Trending"].map((t, i) => (
                <button key={t} className={`px-4 py-1.5 rounded-full transition-colors ${i === 0 ? "bg-[#4040FF] text-white" : "text-[#6B7280] hover:text-[#0F0F1A]"}`}>{t}</button>
              ))}
            </div>
          </div>
        </div>
            ...
            <div key={article.id} className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-shadow duration-300 border border-gray-100 flex flex-col overflow-hidden">
              <div className="p-6 flex-1 flex flex-col">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-xs font-bold text-gray-700">{article.initials}</div>
                    <div>
                      <div className="text-sm font-semibold">{article.author}</div>
                      <div className="text-xs text-[#6B7280]">{article.timeAgo}</div>
                    </div>
                  </div>
                  <span className={`text-xs font-bold px-3 py-1 rounded-full ${article.pathwayColor}`}>{article.pathway}</span>
                </div>
                <h3 className="text-xl font-bold mb-3 leading-snug line-clamp-2">{article.title}</h3>
                <p className="text-[#6B7280] text-sm mb-4 line-clamp-3">{article.excerpt}</p>
                <div className="flex flex-wrap gap-2 mb-6">
                  {article.tags.map(tag => (
                    <span key={tag} className="text-[11px] font-semibold text-[#4040FF] bg-blue-50 px-2.5 py-1 rounded-md">{tag}</span>
                  ))}
                </div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 mb-4">
                    <div className="flex justify-between items-end mb-2">
                      <div className="flex items-center gap-1.5 text-sm font-bold text-gray-700">
                        <Bot size={16} className="text-[#4040FF]" /> Merit Score
                      </div>
              ...
              <div className="bg-gray-50 border-t border-gray-100 p-4 px-6 flex justify-between items-center text-[#6B7280]">
                <div className="flex items-center gap-4 text-sm font-medium">
                  <span className="flex items-center gap-1.5 hover:text-[#4040FF] cursor-pointer transition-colors"><ThumbsUp size={16} /> {article.likes}</span>
                  <span className="flex items-center gap-1.5 hover:text-[#4040FF] cursor-pointer transition-colors"><MessageSquare size={16} /> {article.comments}</span>
                  <span className="flex items-center gap-1.5"><Eye size={16} /> {article.views}</span>
                </div>
                <button className="text-[#4040FF] font-bold text-sm flex items-center gap-1 hover:underline">Read <ChevronRight size={16} /></button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div className="bg-[#0F0F1A] text-white p-6 px-8">
            ...
            <h2 className="text-2xl font-bold">Attention Mechanisms in Biological Neural Networks</h2>
          </div>
          <div className="p-8">
            <div className="space-y-6">
              {comments.map((c) => (
                <div key={c.id} className="flex gap-4 p-5 rounded-xl border border-gray-100 bg-gray-50/50 hover:border-gray-200 transition-colors">
                  <div className="w-10 h-10 rounded-full bg-[#4040FF] text-white flex items-center justify-center text-sm font-bold shrink-0">{c.initials}</div>
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <span className="font-bold text-[#0F0F1A]">{c.author}</span>
                      <span className="text-xs text-[#6B7280]">{c.time}</span>
                    </div>
                    <p className="text-gray-700 text-sm leading-relaxed mb-4">{c.text}</p>
                    <div className="flex items-center gap-6">
                      <div className="flex items-center bg-white border border-gray-200 rounded-full shadow-sm overflow-hidden text-sm">
                        <button className="flex items-center gap-1.5 px-3 py-1.5 hover:bg-gray-50 text-gray-700 font-medium transition-colors border-r border-gray-200">
                          <ArrowUp size={14} className="text-emerald-500" /> {c.upvotes}
                        </button>
                        <button className="flex items-center gap-1.5 px-3 py-1.5 hover:bg-gray-50 text-gray-700 font-medium transition-colors">
                          <ArrowDown size={14} className="text-red-500" /> {c.downvotes}
                        </button>
                      </div>
                      <button className="text-sm font-semibold text-[#6B7280] hover:text-[#4040FF] transition-colors">Reply</button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="mt-8 pt-6 border-t border-gray-100">
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <textarea className="w-full border border-gray-200 rounded-xl p-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#4040FF] focus:border-transparent resize-none h-24 shadow-sm" placeholder="Add a substantive comment to the discussion..." />
                  <div className="flex justify-end mt-3">
                    <button className="bg-[#4040FF] text-white px-6 py-2 rounded-full text-sm font-bold hover:bg-blue-700 transition-colors shadow-sm">Submit Comment</button>
                  </div>
                </div>
              </div>
            </div>
